...
;+(function ($) {
	...
	var Request = function (element, handler, options) {
		...
		var requestOptions = {
			...
			error: function (jqXHR, textStatus, errorThrown) {
				...
				if (
					(window.ocUnloading !== undefined && window.ocUnloading) ||
					errorThrown == 'abort'
				)
					return
				isRedirect = false
			...
			handleUpdateResponse: function (data, textStatus, jqXHR) {
				var updatePromise = $.Deferred().done(function () {
					for (var partial in data) {
						var selector = options.update[partial]
				...
				if (data['X_OCTOBER_ERROR_FIELDS']) {
					requestOptions.handleValidationMessage(
						data['X_OCTOBER_ERROR_MESSAGE'],
						data['X_OCTOBER_ERROR_FIELDS']
					)
				}
				if (data['X_OCTOBER_ASSETS']) {
					assetManager.load(
						data['X_OCTOBER_ASSETS'],
						$.proxy(updatePromise.resolve, updatePromise)
					)
				} else {
					updatePromise.resolve()
				}
	...
	$(window).on(
		'ajaxUpdateComplete',
		function triggerRenderOnAjaxUpdateComplete() {
			$(document).trigger('render')
		}
	)
...
;+(function (window) {
	...
	function getBody(str, pos) {
		...
		if (str[pos] === 't') {
			...
			throw new Error(
				'Broken JSON boolean body near ' + str.substr(0, pos + 10)
			)
		}
		if (str[pos] === 'f') {
		...
		throw new Error(
			'Broken JSON body near ' + str.substr(pos - 5 >= 0 ? pos - 5 : 0, 50)
		)
	}
...
;+(function ($) {
	...
	$(document).on(
		...
		function (event, context, errorMsg, fields) {
